...
function useToggle({
  ...
  React.useEffect(() => {
    warning(!(!hasOnChange && onIsControlled && !readOnly),
      `An \`on\` prop was provided without \`onChange\` handler, this will render read-only toggle. If the field should be mutable use \`initiallOn\`. Otherwise, set either \`onChange\` or \`readOnly\`.`
    )
